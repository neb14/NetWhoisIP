# This Makefile is for the Net::Whois::IP extension to perl.
#
...
#     BUILD_REQUIRES => { Regexp::IPv6=>q[.02] }
#     NAME => q[Net::Whois::IP]
#     PREREQ_PM => { Regexp::IPv6=>q[.02] }
#     VERSION_FROM => q[IP.pm]
...
MAKE = make
FULLEXT = Net/Whois/IP
BASEEXT = IP
PARENT_NAME = Net::Whois
DLBASE = $(BASEEXT)
VERSION_FROM = IP.pm
OBJECT = 
LDFROM = $(OBJECT)
...
INST_LIBDIR      = $(INST_LIB)/Net/Whois
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Net/Whois
...
PM_TO_BLIB = IP.pm \
	$(INST_LIB)/Net/Whois/IP.pm \
	tester.pl \
	$(INST_LIB)/Net/Whois/tester.pl
...
DISTNAME = Net-Whois-IP
DISTVNAME = Net-Whois-IP-1.14
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  IP.pm $(INST_MAN3DIR)/Net::Whois::IP.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Net-Whois-IP' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Regexp::IPv6:  .02' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1.14">' > $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  IP.pm $(INST_LIB)/Net/Whois/IP.pm \
	  tester.pl $(INST_LIB)/Net/Whois/tester.pl 
